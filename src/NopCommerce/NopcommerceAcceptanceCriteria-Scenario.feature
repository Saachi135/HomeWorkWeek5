Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  As a user, I would like to cheak accptance creiteria of Nopcommerce topmanu
  ...
  Scenario: Verify that user can see 'Sort by' filter on Book Category page
    ...
    Then    'Sort by' filter is avaliable on Book Category page
  ...
  Scenario: Verify that user can see 'Name: A to Z' filter is functioning as expected (Note: Products are filters in alphabetical order)
    Given   User is on Books Category page
    When    User is on Books Category page
    And     Click on 'Sort by' Filter
